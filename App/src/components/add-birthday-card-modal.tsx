...
const AddBirthdayCardModal: React.FC<AddBirthdayCardModalProps> = ({
  isVisible,
  onClose,
}) => {
  ...
  const colorBackgrounds = ["bg-blue-400", "bg-purple-600", "bg-teal-500"];
  ...
  const handleAddToQueue = async () => {
    ...
    try {
      ...
      setIsSuccess(false);
      setStatusMessage("Error adding birthday card to queue");
      setTimeout(() => {
        setStatusMessage("");
      }, 3000);
    }
  ...
  return (
    <div className="fixed inset-0 top-17 flex justify-center items-center z-10 bg-gray-800 bg-opacity-50">
      <div className="bg-white p-8 shadow-lg w-full h-full-minus-4.25rem flex flex-col md:flex-row overflow-hidden md:rounded-none relative">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-blue-800 text-4xl"
        >
          &times;
        </button>
        <h2 className="absolute top-24 left-1/2 transform -translate-x-1/2 text-3xl font-bold text-center text-blue-800">
        ...
        <div className="flex flex-col md:flex-row w-full h-full items-center justify-center mt-16">
          <div className="w-full md:w-1/2 p-4 flex flex-col items-center">
            ...
            ></div>
            <div className="w-4/5">
              <label className="block text-lg text-blue-800 font-bold mt-5 mb-2">
                Choose a background
              </label>
          ...
          <div className="w-full md:w-1/2 p-4 flex flex-col items-center md:mt-[-4.75rem]">
            <div className="flex flex-col justify-between h-full w-4/5">
              ...
              <div
                className={`w-full mt-8 text-center ${isSuccess === null ? "" : isSuccess ? "text-green-500" : "text-red-500"}`}
              >
                {statusMessage}
              </div>
            </div>
