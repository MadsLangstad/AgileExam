...
const AddEventCardModal: React.FC<AddEventCardModalProps> = ({ isVisible, onClose }) => {
  ...
  const handleAddToQueue = async () => {
    ...
    try {
      ...
      setIsSuccess(false);
      setStatusMessage('Error adding event card');
      setTimeout(() => setStatusMessage(''), 3000);
    }
  };
  ...
  return (
    <div className="fixed inset-0 top-17 flex justify-center items-center z-10 bg-gray-800 bg-opacity-50">
      <div className="bg-white p-8 shadow-lg w-full h-full-minus-4.25rem flex flex-col md:flex-row overflow-hidden md:rounded-none relative">
        ...
        <h2 className="absolute top-24 left-1/2 transform -translate-x-1/2 text-3xl font-bold text-center text-blue-800">Quick Add Event Card</h2>
