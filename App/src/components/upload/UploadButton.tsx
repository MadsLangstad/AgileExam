...
const UploadButton: React.FC = () => {
    ...
    const handleFileUpload = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:5017/api/upload/upload', formData, {
                headers: {
    ...
    return (
        <div className="flex flex-col items-center justify-center p-4 bg-blue-700 rounded-lg shadow-md space-y-4">
            ...
            {uploadStatus && (
                <p className={`mt-4 ${uploadStatus.startsWith('Error') ? 'text-red-500' : 'text-green-500'}`}>
                    {uploadStatus}
                </p>
            )}
            <div
